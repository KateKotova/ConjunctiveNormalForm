...
clauses
    get_file_data(FileName, Text, _) :-
        ...
        string5x::format(ERROR_MSG, "Невозможно загрузить файл %", FileName), 
        trap(file5x::file_str(FileName, Text), E, (error_handler(ERROR_MSG, E), fail)).
    get_file_data(FN, "", b_true) :- !, 
        string5x::format(Msg, "Файл %s не существует. Создать?", FN), 
        1 = vpiCommonDialogs::messageBox("Prolog Inference Engine", Msg,
...
clauses
    error_handler(S, E) :-
        string5x::format(STR, "% - Код ошибки = %", S, E), 
...
clauses
    ...
    show_menu_with_filelists(MENU, _) :-
        common_exception::raise_error(classInfo, predicate_name(), "Полученное меню не является dynMenu. ResID = ", MENU).
...
clauses
    check_true_item(WIN, id_edit_undo):-    % если буфер отмены не пуст
        vpiEditor::possibleUndoRedo(WIN, IsUndo, _), IsUndo=b_true.
    check_true_item(WIN, id_edit_redo):-    % если буфер повтора не пуст
        vpiEditor::possibleUndoRedo(Win, _, IsRedo), IsRedo=b_true.
    check_true_item(WIN, Item):-    % если блок выделен
...
clauses
    check_false_item(WIN, id_edit_undo):- % если буфер отмены пуст
        vpiEditor::possibleUndoRedo(WIN, IsUndo, _), IsUndo=b_false.
    check_false_item(WIN, id_edit_redo):- % если буфер повтора пуст
        vpiEditor::possibleUndoRedo(Win, _, IsRedo), IsRedo=b_false.
    check_false_item(WIN, Item):-    % блок не выделен
...
clauses
    save_as(_, NewName) :-
        ...
        vpiCommonDialogs::note("Невозможно записать (процесс активен)").
    save_as(_, NewName) :-
        file5x::existfile(NewName), 
        string5x::format(Prompt, "Перезаписать %s?", NewName), 
        Resp = vpiCommonDialogs::ask("Prolog Inference Engine", Prompt, ["Да", "Нет", ""]), 
...
clauses
    clean_up_data(EW, Close) :-
        ...
        !, 
        get_title(FileName, Title), 
        string5x::format(Prompt, "Сохранить %s?", Title), 
        Resp = vpiCommonDialogs::ask("Prolog Inference Engine", Prompt, ["Да", "Нет", "Отмена"]), 
...
clauses
    insert_predicate(EdWin, Pos, What):-
        ...
        trap(BeginPos = vpiEditor::getPos(EdWin), E, (error_handler("невозможно получить текущую позицию", E), fail)), 
        vpiEditor::doLineEnd(EdWin), 
        trap(EndPos = vpiEditor::getPos(EdWin), E1, (error_handler("невозможно получить текущую позицию", E1), fail)), 
        BeginPos = EndPos, !, 
        del_last_enter(What, What1), 
        _ = vpiEditor::pasteStr(EdWin, Pos, What1).
    insert_predicate(EdWin, Pos, What):-
        vpiEditor::doLineEnd(EdWin), 
        trap(EndPos = vpiEditor::getPos(EdWin), E, (error_handler("невозможно получить текущую позицию", E), fail)), 
        Pos = EndPos, !, 
        Pos1 = Pos + 1, 
...
clauses
    save_no_cancel(vpiCommonDialogs::resp_default, FN, Data, b_True) :-
        trap(file5x::file_str(FN, Data), C, (error_handler("Ошибка записи", C), fail)), !, 
...
clauses    
    close_editors(b_False) :-
        ...
        Continue = b_False,  % Нажата ОТМЕНА
        !,
        retractall(app_close).
...
clauses
    ...
    contextMenu(_Point) :-
        common_exception::raise_error(classInfo, predicate_name(), "Контекстное меню не является dynMenu").
...
clauses
    onEngineReconsult(_MenuTag) = handled(0) :-
        ...
        /*#####################################
        % Получение списка лексем с позициями
        ...
        string5x::format(Status, "Перезагрузка %s", FileName), 
        taskWindow::system_status(Status), 
        file5x::write("Перезагружено из: ", FileName), file5x::nl, 
        taskWindow::system_status("Готово").
    onEngineReconsult(_MenuTag) = _ :-
        common_exception::raise_error(classInfo, predicate_name(), "изменить имя файла на факт-переменную").
...
clauses
    onHelpLocal(_MenuTag) = handled(0) :-
        ...
        vpi::showHelpKeyWord("cnf.hlp", Key).
    onHelpLocal(_MenuTag) = handled(0) :-
